"""Assemble the Open Beta 0.88 package from the verified 0.86 assets and adaptive runtime."""
import argparse
import csv
import hashlib
import json
...
def main():
    ...
    package = output / 'AMS2 한국어 패치 오픈베타 0.88'
    ...
    old_config.unlink()
    installer = package / 'AMS2 한국어 패치 오픈베타 0.88.exe'
    shutil.copy2(binaries / 'AMS2-Korean-Patch-OB-0.88.exe', installer)
    shutil.copy2(binaries / 'AMS2-Korean-Patch-OB-0.88.exe.config', package / (installer.name + '.config'))
    shutil.copy2(REPO / 'releases/0.88/RELEASE_NOTES.md', package / 'RELEASE_NOTES.md')
    shutil.copy2(REPO / 'installer/0.88/README.md', package / 'README.md')
    shutil.copy2(REPO / 'installer/0.88/ownership-coverage.json', package / 'manifest/ownership-coverage.json')
    ...
    metadata = {
        ...
        'ownership_catalog_sha256': sha(REPO / 'installer/0.88/ownership.tsv'),
    ...
    try:
        regression = release_regressions.verify(package, compatibility, '0.88', package / 'manifest/regression-report.json')
    except Exception:
