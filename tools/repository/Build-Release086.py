"""Assemble the Open Beta 0.86 package from the verified 0.85 assets and adaptive runtime."""
import argparse
import csv
import hashlib
import json
...
def main():
    ...
    baseline = WORK / 'releases/0.85/AMS2 한국어 패치 오픈베타 0.85'
    package = output / 'AMS2 한국어 패치 오픈베타 0.86'
    ...
    old_installer = package / 'AMS2 한국어 패치 오픈베타 0.85.exe'
    old_config = package / 'AMS2 한국어 패치 오픈베타 0.85.exe.config'
    old_installer.unlink()
    old_config.unlink()
    installer = package / 'AMS2 한국어 패치 오픈베타 0.86.exe'
    shutil.copy2(binaries / 'AMS2-Korean-Patch-OB-0.86.exe', installer)
    shutil.copy2(binaries / 'AMS2-Korean-Patch-OB-0.86.exe.config', package / (installer.name + '.config'))
    shutil.copy2(REPO / 'releases/0.86/RELEASE_NOTES.md', package / 'RELEASE_NOTES.md')
    ...
    try:
        regression = release_regressions.verify(package, compatibility, '0.86', package / 'manifest/regression-report.json')
    except Exception:
