    ...
    internal static class GameUpdateCompatibility
    {
        ...
        internal static void Ensure(string game, bool requireInstalled, Action<string> log = null)
        {
            ...
            using (var mutex = new Mutex(true, @"Local\AMS2-Korean-GameUpdate-" + FileOps.Sha256Text(Path.GetFullPath(game).ToUpperInvariant()).Substring(0, 24), out owner))
            {
                ...
                ContentManagerOverlay.Ensure(game, root, log);
                ...
                if (rule.helpOverrides != null)
                    foreach (var pair in rule.helpOverrides)
                    {
                        ...
                        if (before.End + 12 > source.Length || current.End + 12 > candidate.Length ||
                            Hash(source.Skip(before.Record).Take(before.Start - before.Record).ToArray()) != fix[0] ||
                            source[before.Record + 57] != Byte.Parse(fix[1], CultureInfo.InvariantCulture) ||
                            BitConverter.ToUInt32(source, before.End + 8) != UInt32.Parse(fix[3], NumberStyles.HexNumber, CultureInfo.InvariantCulture))
                            throw new InvalidDataException("헤일로 도움말 연결이 검증된 원본과 다릅니다.");
                        candidate[current.Record + 57] = Byte.Parse(fix[2], CultureInfo.InvariantCulture);
                        Buffer.BlockCopy(BitConverter.GetBytes(UInt32.Parse(fix[4], NumberStyles.HexNumber, CultureInfo.InvariantCulture)), 0, candidate, current.End + 8, 4);
                    }
        ...
        private static byte[] ReplaceLiteral(byte[] data, string before, string after)
        {
            ...
            if (found < 0) throw new InvalidDataException("메뉴 설명 원문이 변경되었습니다. 최신 호환 패치가 필요합니다.");
            using (var output = new MemoryStream())
            {
