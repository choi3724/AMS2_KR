...
{
    // Shared by both launchers and the installer. Build IDs are advisory; file structure decides.
    internal static class GameUpdateCompatibility
    {
        public sealed class MenuRule
        {
            ...
            public Dictionary<string, string[]> namedOverrides;
            public Dictionary<string, string[]> helpOverrides;
        ...
        public sealed class Policy { public int schema; public bool buildIsAdvisory, requiredDirectFiles; public string hashPurpose; public string[] optionalArchives; }
        internal sealed class CompatibilityOutcome
        {
            public string BuildId;
            public bool UnknownBuild, BuildChanged;
            public int Reapplied, SkippedOptional;
            public readonly List<string> Warnings = new List<string>();
            // CM may regenerate shared files at every launch. Successful repair
            // on the same build is routine; only a new build or warning needs acknowledgement.
            public bool ShouldNotify { get { return UnknownBuild && BuildChanged || SkippedOptional > 0; } }
            public string Notice
            {
                get
                {
                    var lines = new List<string>();
                    if (UnknownBuild && BuildChanged) lines.Add("새 게임 빌드 " + BuildId + "을 감지했습니다. 빌드 번호만으로 실행을 차단하지 않고 현재 파일 구조를 검사했습니다.");
                    if (Reapplied > 0) lines.Add("한글패치 " + Reapplied + "개 항목을 현재 게임 파일에 다시 적용했습니다.");
                    if (SkippedOptional > 0) lines.Add("부가 표시·계기판 " + SkippedOptional + "개 항목은 안전하게 적용할 수 없어 건너뛰었습니다. 핵심 한글 자산은 유효하므로 한글 모드로 실행합니다.");
                    lines.AddRange(Warnings.Take(8));
                    lines.Add("Steam 실행 요청 성공과 실제 게임 내 한글 표시는 별도입니다.");
                    return String.Join("\r\n", lines);
                }
            }
        }
        private sealed class Change { public string Relative, Target, Before, After; public byte[] Content; public bool WriteTarget = true, PreserveTimes; public long Creation, Modified; }
        private sealed class Snapshot { public string Target, Copy, Hash; public FileAttributes Attributes; public long Creation, Modified; }
        private sealed class TextField { public int Start, End, Record; public uint Id; public string Font, Key; }
        ...
        internal static string ActiveState(string game)
        {
            ...
            foreach (string folder in UpgradeOwnership.Roots(game))
            {
                string file = SafePath(folder, "install-state.tsv");
                if (!File.Exists(file)) continue;
                string status;
                if (!UpgradeOwnership.Head(folder).TryGetValue("status", out status)) continue;
                if (status == "INSTALLED" || status == "GAME_UPDATE_PENDING") active.Add(folder);
            }
            var history = UpgradeOwnership.Read(game, null);
            return active.OrderByDescending(x => UpgradeOwnership.Head(x)["status"] == "GAME_UPDATE_PENDING")
                .ThenByDescending(x => history.Where(h => h.StateRoot == x).Sum(h => h.State.Files.Count(f => UpgradeOwnership.Match(game, f))))
                .ThenByDescending(x => File.GetLastWriteTimeUtc(Path.Combine(x, "install-state.tsv"))).FirstOrDefault();
        }
        ...
        internal static bool NeedsRepair(string game)
        {
            ...
            foreach (string line in File.ReadLines(Path.Combine(root, "files.tsv")).Skip(1))
            {
                ...
                if (row.Length != 9 && row.Length != 6) throw new InvalidDataException("설치 기록 행이 손상되었습니다: " + row[0]);
        ...
        internal static bool RecoverInterruptedOnly(string game, Action<string> log)
        {
            bool recovered = false;
            foreach (string root in UpgradeOwnership.Roots(game))
            {
                var head = UpgradeOwnership.Head(root);
                string status;
                if (head.TryGetValue("status", out status) && status == "GAME_UPDATE_PENDING")
                {
                    GuardGame();
                    RecoverPending(game, root, head, ForBuild(SteamLocator.FromGameDirectory(game).BuildId));
                    recovered = true;
                    if (log != null) log("이전 게임 업데이트 중단 작업 복구 완료: " + root);
                }
                bool pending = File.Exists(Path.Combine(root, "cm-overlay", "pending.json"));
                ContentManagerOverlay.RecoverPendingOnly(game, root, log);
                recovered |= pending;
            }
            return recovered;
        }

        internal static CompatibilityOutcome Ensure(string game, bool requireInstalled, Action<string> log = null)
        {
            bool owner;
            using (var mutex = new Mutex(true, @"Local\AMS2-Korean-GameUpdate-" + FileOps.Sha256Text(Path.GetFullPath(game).ToUpperInvariant()).Substring(0, 24), out owner))
            {
                ...
                UpgradeTransaction.Recover(game, log);
                GameInfo info = SteamLocator.FromGameDirectory(game);
                Rules rules = ForBuild(info.BuildId);
                var outcome = new CompatibilityOutcome { BuildId = info.BuildId, UnknownBuild = !SupportedBuild(info.BuildId) };
                string textIndex = SafePath(game, "Pakfiles/Dir/TEXT.bff");
                if (!TextIndexMatches(textIndex, rules) && log != null)
                    log("참고: Steam 번역 인덱스가 기존 검증값과 다릅니다. 빌드 차단 대신 loose 번역 자산과 대상 파일 구조를 검사합니다.");
                string root = ActiveState(game);
                if (root == null)
                {
                ...
                outcome.BuildChanged = head["buildid"] != info.BuildId;
                ...
                ContentManagerOverlay.RecoverPendingOnly(game, root, log);
                string cmState = ContentManagerOverlay.StateToken(game);
                HashSet<string> cmOwned = ContentManagerOverlay.Owned(game);
                ...
                if (rows.Any(x => x.Length != 9 && x.Length != 6) || rows.Select(x => x[0].Replace('\\', '/').ToLowerInvariant()).Distinct().Count() != rows.Count)
                    throw new InvalidDataException("설치 파일 기록에 손상 또는 중복 행이 있습니다.");
                rows = rows.Select(x => x.Length == 6 ? x.Concat(new[] { x[1] == "modified" ? "1" : "0", x[2], x[3] }).ToArray() : x).ToList();
                var changes = new List<Change>();
                var failures = new List<string>();
                foreach (string[] row in rows)
                {
                    string relative = row[0].Replace('\\', '/').ToLowerInvariant();
                    if (!rules.files.ContainsKey(relative))
                    {
                        string oldFile = SafePath(game, relative);
                        if (File.Exists(oldFile) && UpgradeOwnership.Official(relative.Replace('/','\\'), FileOps.Sha256(oldFile), new FileInfo(oldFile).Length)) continue;
                        failures.Add(relative + " | 이전 관리 파일의 소유권 또는 실제 파일 확인 실패"); continue;
                    }
                    string live = SafePath(game, relative);
                    if (!File.Exists(live)) { failures.Add(relative + " | 필수 파일 누락"); continue; }
                    string current = FileOps.Sha256(live);
                    if (current == row[5]) continue;
                    byte[] source = File.ReadAllBytes(live), candidate = null;
                    if (Hash(source) != current) { failures.Add(relative + " | 검사 중 동시 변경 감지"); continue; }
                    try
                    {
                        if (cmOwned.Contains(relative))
                        {
                            if (!ContentManagerOverlay.TryPatchCurrent(relative, source, out candidate))
                                throw new InvalidDataException("CM 공유 파일의 구조 기반 한글 규칙이 없습니다.");
                        }
                        else if (rules.menus.ContainsKey(relative)) candidate = PatchMenu(source, rules.menus[relative]);
                        else if (relative == "pakfiles/igphasehud.bff" || relative == "pakfiles/huddisplay.bff")
                            candidate = PatchArchive(game, root, relative, source);
                        else throw new InvalidDataException("이 파일 형식의 구조 기반 재적용 규칙이 없습니다.");
                    }
                    catch (Exception error)
                    {
                        bool optional = IsOptional(rules, relative);
                        if (!optional) { failures.Add(relative + " | 재적용 시도 실패 | " + error.Message); continue; }
                        outcome.SkippedOptional++;
                        outcome.Warnings.Add(relative + " | 부가 항목 재적용 생략 | " + error.Message);
                        // Adopt the new untouched game file as both original and current
                        // state so it is never rolled back to an older game build.
                        changes.Add(new Change { Relative = relative, Target = live, Before = current, After = current, Content = source, WriteTarget = false,
                            PreserveTimes = cmOwned.Contains(relative), Creation = File.GetCreationTimeUtc(live).Ticks, Modified = File.GetLastWriteTimeUtc(live).Ticks });
                        continue;
                    }
                    if (Hash(source) == current && Hash(candidate) == current)
                    {
                        // Structurally valid and already patched.  Refresh only the state.
                        changes.Add(new Change { Relative = relative, Target = live, Before = current, After = current, Content = candidate, WriteTarget = false,
                            PreserveTimes = cmOwned.Contains(relative), Creation = File.GetCreationTimeUtc(live).Ticks, Modified = File.GetLastWriteTimeUtc(live).Ticks });
                    }
                    else
                    {
                ...
                if (failures.Count > 0)
                {
                    ...
                    if (failures.Count > 40) detail += "\r\n... 외 " + (failures.Count - 40) + "개";
                    throw new InvalidDataException("필수 한글 자산의 재적용 가능성을 확인하지 못했습니다. 게임 파일은 변경하지 않았습니다.\r\n" + detail);
                }
                if (changes.Count == 0 && !outcome.BuildChanged)
                {
                    if (outcome.Warnings.Count > 0 && log != null) foreach (string warning in outcome.Warnings) log(warning);
                    return outcome;
                }
                GuardGame();
                if (ContentManagerOverlay.StateToken(game) != cmState) throw new IOException("후보 생성 중 Content Manager 상태가 변경되었습니다.");
                if (log != null) log("게임 업데이트 감지: 전체 관리 파일을 검사하고 현재 파일에서 한글 변경 후보를 생성했습니다.");
                Apply(game, root, headPath, filesPath, head, "relative_path\taction\tbefore_bytes\tbefore_sha256\tafter_bytes\tafter_sha256\tinstall_before_exists\tinstall_before_bytes\tinstall_before_sha256", rows, info, changes);
                if (log != null)
                {
                    log("게임 업데이트 대응 완료: 재적용 " + outcome.Reapplied + "개, 부가 항목 생략 " + outcome.SkippedOptional + "개");
                    foreach (string warning in outcome.Warnings) log(warning);
                }
                return outcome;
            }
        }

        private static bool IsOptional(Rules rules, string relative)
        {
            return rules.policy != null && rules.policy.optionalArchives != null &&
                rules.policy.optionalArchives.Any(x => x.Equals(relative, StringComparison.OrdinalIgnoreCase));
        }

        private static byte[] PatchArchive(string game, string root, string relative, byte[] source)
        {
            string tool = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "runtime", "AMS2.DynamicBffPatcher.exe");
            if (!File.Exists(tool)) tool = Path.Combine(root, "runtime", "AMS2.DynamicBffPatcher.exe");
            if (!File.Exists(tool)) throw new FileNotFoundException("동적 아카이브 적용기가 없습니다.", tool);
            string folder = SafePath(root, "game-updates/candidate-" + Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(folder);
            try
            {
                string input = Path.Combine(folder, "source.bff"), output = Path.Combine(folder, "candidate.bff"), report = Path.Combine(folder, "report.json");
                File.WriteAllBytes(input, source);
                FileOps.RunBffTool(tool, relative.EndsWith("huddisplay.bff", StringComparison.OrdinalIgnoreCase) ? "patch-huddisplay" : "patch", game, input, output, report);
                if (!File.Exists(output)) throw new InvalidDataException("동적 아카이브 후보가 생성되지 않았습니다.");
        ...
        {
            return new InvalidDataException("현재 게임 빌드: " + info.BuildId + "\r\n확인할 항목: " + file + "\r\n현재 파일의 구조 기반 재적용 가능성을 확인하지 못했습니다. 빌드 번호나 해시 불일치만으로 차단한 것이 아니며, 안전한 후보를 만들 수 없어 해당 작업을 중단했습니다.");
        }
        ...
        private static void Apply(string game, string root, string headPath, string filesPath, Dictionary<string, string> head, string header, List<string[]> rows, GameInfo info, List<Change> changes)
        {
            ...
            foreach (Change change in changes)
            {
                ...
                if (row[1] != "modified" && row[1] != "created") throw new InvalidDataException("알 수 없는 설치 파일 역할입니다: " + row[0]);
                if (row[1] == "modified")
                {
                ...
                if (change.WriteTarget) writes.Add(change);
            ...
            foreach (string target in writes.Select(x => x.Target).Concat(new[] { headPath }).Distinct(StringComparer.OrdinalIgnoreCase))
            {
                ...
                snapshots.Add(new Snapshot { Target = target, Copy = backup, Hash = hash, Attributes = File.GetAttributes(target),
                    Creation = File.GetCreationTimeUtc(target).Ticks, Modified = File.GetLastWriteTimeUtc(target).Ticks });
            }
            ...
            try
            {
                ...
                foreach (Change write in writes)
                {
                    ...
                    if (write.PreserveTimes)
                    {
                        File.SetCreationTimeUtc(write.Target, new DateTime(write.Creation, DateTimeKind.Utc));
                        File.SetLastWriteTimeUtc(write.Target, new DateTime(write.Modified, DateTimeKind.Utc));
                    }
            ...
            catch (Exception error)
            {
                ...
                foreach (Snapshot snapshot in touched.AsEnumerable().Reverse())
                    try
                    {
                        ...
                        File.SetCreationTimeUtc(snapshot.Target, new DateTime(snapshot.Creation, DateTimeKind.Utc));
                        File.SetLastWriteTimeUtc(snapshot.Target, new DateTime(snapshot.Modified, DateTimeKind.Utc));
        ...
        private static void RecoverPending(string game, string root, Dictionary<string, string> head, Rules rules)
        {
            ...
            foreach (string relative in rules.files.Keys.Concat(rules.archives.Keys))
            {
        ...
        private static List<TextField> Parse(byte[] data)
        {
            ...
            // Several valid in-car display BGUI files have no sprite resources.
            if (Single.IsNaN(scale) || Single.IsInfinity(scale) || count > 4096) throw new InvalidDataException("메뉴 헤더 변경");
            int cursor = 8;
            ...
            const string namedNodeName = "KERSDeploymentModeName";
            byte[] namedBytes = Utf8.GetBytes(namedNodeName);
            for (int at = 0; at <= data.Length - namedBytes.Length - 1; at++)
            {
                if (data[at] != namedBytes.Length) continue;
                bool match = true;
                for (int n = 0; n < namedBytes.Length; n++) if (data[at + 1 + n] != namedBytes[n]) { match = false; break; }
                if (!match) continue;
                int start = at - 4, afterName = at + 1 + namedBytes.Length;
                if (start < 0 || afterName + 74 >= data.Length || BitConverter.ToUInt32(data, start) > 65535) continue;
                uint objectId = BitConverter.ToUInt32(data, afterName + 4);
                TextField field = fields.SingleOrDefault(x => x.Start == afterName + 73);
                if (field != null && objectId > 0 && objectId <= 1000000) field.Key = namedNodeName + ":" + objectId.ToString(CultureInfo.InvariantCulture);
            }
            return fields;
        }
        ...
        internal static byte[] PatchMenu(byte[] source, MenuRule rule)
        {
            ...
            using (var output = new MemoryStream())
            {
                ...
                foreach (TextField record in records)
                {
                    ...
                    string[] namedSpecial;
                    if (!String.IsNullOrEmpty(record.Key) && rule.namedOverrides != null && rule.namedOverrides.TryGetValue(record.Key, out namedSpecial))
                    {
                        if (namedSpecial.Length != 2 || (!record.Font.Equals(namedSpecial[0], StringComparison.OrdinalIgnoreCase) && !record.Font.Equals(namedSpecial[1], StringComparison.OrdinalIgnoreCase)))
                            throw new InvalidDataException("개별 계기판 글씨 구조가 바뀌었습니다: " + record.Key);
                        target = namedSpecial[1];
                    }
                    string[] special;
                    if (rule.overrides.TryGetValue(record.Id.ToString(), out special))
                    {
                        if (special.Length != 3 || (!record.Font.Equals(special[0], StringComparison.OrdinalIgnoreCase) && !record.Font.Equals(special[1], StringComparison.OrdinalIgnoreCase)) ||
                            (record.Font.Equals(special[0], StringComparison.OrdinalIgnoreCase) && Hash(source.Skip(record.Record).Take(record.Start - record.Record).ToArray()) != special[2]))
                ...
                if (rule.helpOverrides != null)
                    foreach (var pair in rule.helpOverrides)
                    {
                        int index = records.FindIndex(r => r.Record >= 0 && r.Id.ToString() == pair.Key);
                        string[] fix = pair.Value;
                        if (index < 0 || fix.Length != 5) throw new InvalidDataException("헤일로 도움말 구조가 변경되었습니다.");
                        TextField before = records[index], current = after[index];
                        if (before.End + 12 > source.Length || current.End + 12 > candidate.Length)
                            throw new InvalidDataException("헤일로 도움말 연결이 검증된 원본과 다릅니다.");
                        byte oldByte = Byte.Parse(fix[1], CultureInfo.InvariantCulture), newByte = Byte.Parse(fix[2], CultureInfo.InvariantCulture);
                        uint oldHash = UInt32.Parse(fix[3], NumberStyles.HexNumber, CultureInfo.InvariantCulture), newHash = UInt32.Parse(fix[4], NumberStyles.HexNumber, CultureInfo.InvariantCulture);
                        byte actualByte = source[before.Record + 57]; uint actualHash = BitConverter.ToUInt32(source, before.End + 8);
                        if (actualByte == oldByte && actualHash == oldHash)
                        {
                            if (Hash(source.Skip(before.Record).Take(before.Start - before.Record).ToArray()) != fix[0]) throw new InvalidDataException("헤일로 도움말 항목 구조가 변경되었습니다.");
                            candidate[current.Record + 57] = newByte;
                            Buffer.BlockCopy(BitConverter.GetBytes(newHash), 0, candidate, current.End + 8, 4);
                        }
                        else if (actualByte != newByte || actualHash != newHash) throw new InvalidDataException("헤일로 도움말 연결이 검증된 원본과 다릅니다.");
                    }
                if (rule.literalText != null)
                {
                    if (rule.literalText.Length != 2) throw new InvalidDataException("메뉴 설명 번역 규칙이 잘못되었습니다.");
                    candidate = ReplaceLiteral(candidate, rule.literalText[0], rule.literalText[1]);
                }
        ...
        private static byte[] ReplaceLiteral(byte[] data, string before, string after)
        {
            byte[] pattern = BitConverter.GetBytes(before.Length).Concat(Encoding.Unicode.GetBytes(before)).ToArray();
            byte[] replacement = BitConverter.GetBytes(after.Length).Concat(Encoding.Unicode.GetBytes(after)).ToArray();
            int found = -1;
            for (int at = 0; at <= data.Length - pattern.Length; at++)
            {
                int j = 0;
                while (j < pattern.Length && data[at + j] == pattern[j]) j++;
                if (j != pattern.Length) continue;
                if (found >= 0) throw new InvalidDataException("메뉴 설명이 중복되어 자동 번역을 중단했습니다.");
                found = at;
            }
            if (found < 0)
            {
                int existing = -1;
                for (int at = 0; at <= data.Length - replacement.Length; at++)
                {
                    int j = 0; while (j < replacement.Length && data[at + j] == replacement[j]) j++;
                    if (j != replacement.Length) continue;
                    if (existing >= 0) throw new InvalidDataException("번역된 메뉴 설명이 중복되어 자동 검증을 중단했습니다.");
                    existing = at;
                }
                if (existing >= 0) return data;
                throw new InvalidDataException("메뉴 설명 원문이 변경되었습니다. 구조 검토가 필요합니다.");
            }
            using (var output = new MemoryStream())
            {
                output.Write(data, 0, found);
                output.Write(replacement, 0, replacement.Length);
                output.Write(data, found + pattern.Length, data.Length - found - pattern.Length);
