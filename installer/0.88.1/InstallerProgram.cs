...
namespace Ams2KoreanBeta
{
    ...
    internal sealed class InstallerForm : Form
    {
        ...
        public InstallerForm()
        {
            ...
            Text = "Automobilista 2 한국어 패치 — Open Beta 0.88.1";
            Font = UiFont(9F);
        ...
        private Control BuildTitleBar()
        {
            ...
            Label title = new Label { Text = "Automobilista 2 한국어 패치 — Open Beta 0.88.1", ForeColor = TextPrimary, Font = UiFont(10.5F), TextAlign = ContentAlignment.MiddleLeft, Location = new Point(49, 0), Size = new Size(720, 43) };
        ...
        private Control BuildFooter()
        {
            ...
            Label version = new Label { Text = "Automobilista 2 한국어 패치 — Open Beta 0.88.1", ForeColor = Color.FromArgb(131, 118, 117), Font = UiFont(8.5F), TextAlign = ContentAlignment.MiddleRight, Anchor = AnchorStyles.Top | AnchorStyles.Right, Size = new Size(430, 37) };
        ...
        private Control BuildBody()
        {
            ...
            RowStyle statusRow = new RowStyle(SizeType.Absolute, 84F);
            body.RowStyles.Add(statusRow);
            ...
            {
                const float statusArtworkRatio = 2800F / 176F;
                float fittedHeight = body.ClientSize.Width / statusArtworkRatio + ScalePx(body, 8);
                statusRow.Height = Math.Max(ScalePx(body, 68), Math.Min(ScalePx(body, 84), (float)Math.Round(fittedHeight)));
            };
            return body;
        }
        ...
        private Control BuildStatusCard()
        {
            ...
            {
                float verticalScale = Math.Min(1F, card.ClientSize.Height / (float)Math.Max(1, ScalePx(card, 88)));
                int badgeTop = (int)Math.Round(ScalePx(card, 18) * verticalScale);
                int badgeHeight = Math.Max(ScalePx(card, 19), (int)Math.Round(ScalePx(card, 28) * verticalScale));
                int detailTop = (int)Math.Round(ScalePx(card, 47) * verticalScale);
                int detailHeight = Math.Max(ScalePx(card, 16), (int)Math.Round(ScalePx(card, 24) * verticalScale));
                statusBadge.SetBounds(ScalePx(card, 92), badgeTop, ScalePx(card, 520), badgeHeight);
                statusDetail.SetBounds(ScalePx(card, 92), detailTop, Math.Max(ScalePx(card, 300), card.ClientSize.Width - ScalePx(card, 700)), detailHeight);
                int versionLeft = (int)Math.Round(card.ClientSize.Width * 0.888F);
                int versionRight = Math.Max(ScalePx(card, 12), (int)Math.Round(card.ClientSize.Width * 0.017F));
                int versionWidth = Math.Max(ScalePx(card, 90), card.ClientSize.Width - versionLeft - versionRight);
                int versionTop = (int)Math.Round(ScalePx(card, 43) * verticalScale);
                int versionHeight = Math.Max(ScalePx(card, 18), (int)Math.Round(ScalePx(card, 28) * verticalScale));
                patchVersion.SetBounds(Math.Max(0, versionLeft), versionTop, versionWidth, versionHeight);
        ...
        {
            DialogResult answer = MessageBox.Show(this, "한국어 패치가 덮어쓴 파일은 설치 직전에 자동 백업한 파일로 복원하고, 패치가 새로 만든 파일과 바로가기는 제거합니다.\n\n복원 후 파일 해시까지 검증합니다. 계속하시겠습니까?", "설치 직전 상태로 복원", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
        ...
        private void Run(string action)
        {
            ...
            {
                using (SaveFileDialog d = new SaveFileDialog()) { d.Filter = "ZIP 파일|*.zip"; d.FileName = "AMS2-Korean-Beta07-Diagnostic-" + DateTime.Now.ToString("yyyyMMdd-HHmmss") + ".zip"; if (d.ShowDialog(this) != DialogResult.OK) return; diag = d.FileName; }
            }
        ...
        private static Color StatusBackColor(string value, bool error)
        {
            ...
            if (value == "INSTALLED_WITH_WARNINGS" || value == "UPDATED_WITH_WARNINGS" || value == "UPDATE_AVAILABLE" || value == "WORKING" || value == "INSTALLABLE_EXISTING_STATE") return Color.FromArgb(68, 49, 18);
        ...
        private static Color StatusColor(string value)
        {
            ...
            if (value == "INSTALLED_WITH_WARNINGS" || value == "UPDATED_WITH_WARNINGS" || value == "UPDATE_AVAILABLE" || value == "WORKING" || value == "INSTALLABLE_EXISTING_STATE") return Warning;
        ...
        {
            if (value == "GAME_UPDATE_DETECTED") return "게임 업데이트 / 파일 변경 감지"; if (value == "UPDATE_AVAILABLE") return "업데이트 필요"; if (value == "UPDATED_EXACT") return "한국어 패치 업데이트 완료"; if (value == "INSTALLED_EXACT") return "한국어 패치 설치 완료"; if (value == "UPDATED_WITH_WARNINGS") return "업데이트 완료 (부가 항목 생략)"; if (value == "INSTALLED_WITH_WARNINGS") return "설치 완료 (부가 항목 생략)"; if (value == "RESTORED_EXACT") return "설치 직전 상태 복원 완료"; if (value == "RESTORE_FAILED") return "자동 복구 실패"; if (value == "STEAM_LAUNCH_FAILED") return "Steam 실행 요청 실패"; if (value == "STOCK_ENGLISH") return "한국어 패치 미설치"; if (value == "INSTALLABLE_EXISTING_STATE") return "기존 파일 백업 후 설치 가능"; if (value == "MIXED_OR_INCOMPLETE") return "혼합 / 불완전 상태"; if (value == "BACKUP_MISSING_OR_DAMAGED") return "백업 손상 / 누락"; if (value == "MANUAL_CHANGES_DETECTED") return "수동 변경 감지"; if (value == "NOT_INSTALLED") return "한글 패치 미설치"; if (value == "LAUNCHED_KOREAN") return "한국어 게임 실행 완료"; if (value == "DIAGNOSTIC_CREATED") return "진단 ZIP 생성 완료"; if (value == "WORKING") return "처리 중"; if (value == "PATH_REQUIRED") return "경로 선택 필요"; if (value == "PACKAGE_ERROR") return "패키지 오류"; return value;
        }

        private void ApplyInstallButton(string state) { bool update = state == "UPDATE_AVAILABLE" || state == "GAME_UPDATE_DETECTED"; SetButtonArtwork(install, update ? "install-update" : "install", update ? "↓  업데이트" : "↓  설치"); }
        private static string DisplayText(string value) { return (value ?? "").Replace("INSTALLED_EXACT", "한글 패치 설치 완료").Replace("RESTORED_EXACT", "설치 직전 상태 복원 완료").Replace("STOCK_ENGLISH", "한국어 패치 미설치").Replace("INSTALLABLE_EXISTING_STATE", "기존 파일 백업 후 설치 가능").Replace("NOT_INSTALLED", "한글 패치 미설치"); }
