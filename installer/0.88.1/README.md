...
0.88.1은 같은 게임 빌드에서 정상 한글 재적용 안내가 매번 표시되는 문제를 수정했습니다. 새 빌드 첫 감지와 적용 실패 경고는 유지합니다.
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File tools/repository/Build-InstallerOutsideRepo.ps1 -Version 0.88.1 -WorkRoot <작업 경로> -CompatibilityDataRoot <검증된 호환성 데이터> -VerifiedBffPatcher <정식 0.86의 변경 없는 BFF 적용기>
```

정식 배포 식별 목록 생성과 검증 범위는 `docs/RELEASE_0.88.1_VALIDATION.md`에 기록한다.
