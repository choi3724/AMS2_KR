...
namespace Ams2KoreanBeta
{
    ...
    {
        public const string PackageId = "AMS2-KR-BETA-0.6.6-PRETENDARD";
        public const string Version = "Closed Beta 0.6.6";
    ...
    internal sealed class BetaEngine
    {
        ...
        public OperationResult Diagnose(GameInfo game, string outputZip)
        {
            try
            {
                Prepare(game);
                ValidateGame(game);
                string temp = Path.Combine(Path.GetTempPath(), "AMS2-KR-BETA066-DIAG-" + Guid.NewGuid().ToString("N"));
        ...
        private InstallState BuildState(GameInfo game)
        {
            ...
            string bffLive = PackageManifest.SafeJoin(gameDir, PackageManifest.IgphaseHudRelativePath);
            DirectState bffState = new DirectState {
                RelativePath = PackageManifest.IgphaseHudRelativePath,
                Action = "modified",
                BeforeSha = FileOps.Sha256(bffLive),
                BeforeBytes = new FileInfo(bffLive).Length,
                AfterSha = PackageManifest.IgphaseHudTargetSha256,
                AfterBytes = PackageManifest.IgphaseHudBytes
            };
            string bffBackup = PackageManifest.SafeJoin(backupRoot, PackageManifest.IgphaseHudRelativePath);
            if (File.Exists(bffBackup)) PackageManifest.RequireFile(bffBackup, bffState.BeforeBytes, bffState.BeforeSha, "기존 IGPHASEHUD backup");
            else FileOps.CopyNewExact(bffLive, bffBackup, bffState.BeforeSha);
            s.Files.Add(bffState);
            return s;
        }
