...
namespace Ams2KoreanBeta
{
    internal static class GameLauncher
    {
        ...
        public static void Start(string game, bool vr)
        {
            ...
            GameUpdateCompatibility.CompatibilityOutcome outcome = GameUpdateCompatibility.Ensure(game, true);
            if (outcome.ShouldNotify)
                MessageBox.Show(outcome.Notice, "AMS2 한글패치 게임 업데이트 대응", MessageBoxButtons.OK, outcome.SkippedOptional > 0 ? MessageBoxIcon.Warning : MessageBoxIcon.Information);
            StartVerified(game, vr);
        }

        public static void StartVerified(string game, bool vr)
        {
            string steam = FindSteam();
            try
            {
                Process launched = Process.Start(new ProcessStartInfo(steam, Arguments(vr)) { WorkingDirectory = Path.GetDirectoryName(steam), UseShellExecute = true });
                if (launched == null) throw new InvalidOperationException("Steam 프로세스 시작 결과가 없습니다.");
                launched.Dispose();
            }
            catch (Exception error) { throw new InvalidOperationException("Steam 실행 요청 실패: " + error.Message, error); }
        }
    }
}
