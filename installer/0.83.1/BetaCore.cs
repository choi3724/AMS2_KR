...
namespace Ams2KoreanBeta
{
    ...
    {
        public const string PackageId = "AMS2-KR-BETA-0.83.1-PRETENDARD";
        public const string Version = "Open Beta 0.83.1";
    ...
    internal sealed class BetaEngine
    {
        ...
        private void ValidateGame(GameInfo game)
        {
            ...
            if (!GameUpdateCompatibility.SupportedBuild(game.BuildId) && GameUpdateCompatibility.ActiveState(gameDir) == null)
                throw GameUpdateCompatibility.Unsupported(game, "게임 버전");
            Log("게임 빌드 " + game.BuildId + "에 맞는 패치 파일을 선택했습니다.");
        ...
        private void PreflightFiles()
        {
            ...
            manifest.Ers.Prepare(manifest.ReleaseRoot, PackageManifest.SafeJoin(gameDir, ErsArchivePatch.RelativePath), ersCandidatePath, ersOriginalPath);
        ...
        private InstallState BuildState(GameInfo game, PreviousInstallInfo previous)
        {
            ...
            ResolveCanonicalOriginal(bff, false, new[] { GameUpdateCompatibility.ForBuild(game.BuildId).archives["pakfiles/igphasehud.bff"].stock }, history, s.BackupId);
            s.Files.Add(bff);
            DirectState ers = CreateTransactionRecord(ErsArchivePatch.RelativePath, manifest.Ers.PatchedSha, manifest.Ers.ArchiveBytes, s.BackupId);
            if (ers.InstallBeforeSha == manifest.Ers.PatchedSha && !history.Any(item => item.State.Files.Any(old =>
                old.RelativePath.Equals(ers.RelativePath, StringComparison.OrdinalIgnoreCase) && old.AfterSha == ers.InstallBeforeSha)))
            {
                ...
                ers.Action = "modified"; ers.BeforeSha = manifest.Ers.StockSha; ers.BeforeBytes = manifest.Ers.ArchiveBytes;
                CopyCanonicalBackup(ersOriginalPath, ers, s.BackupId);
            }
            else ResolveCanonicalOriginal(ers, false, new[] { manifest.Ers.StockSha }, history, s.BackupId);
            s.Files.Add(ers);
