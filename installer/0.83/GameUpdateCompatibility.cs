    ...
    internal static class GameUpdateCompatibility
    {
        ...
        internal static void Ensure(string game, bool requireInstalled, Action<string> log = null)
        {
            ...
            using (var mutex = new Mutex(true, @"Local\AMS2-Korean-GameUpdate-" + FileOps.Sha256Text(Path.GetFullPath(game).ToUpperInvariant()).Substring(0, 24), out owner))
            {
                ...
                if (FileOps.Sha256(SafePath(game, "Pakfiles/Dir/TEXT.bff")) != rules.textIndex)
        ...
        private static Exception Unsupported(GameInfo info, string file)
        {
            return new InvalidDataException("게임 업데이트 또는 파일 변경이 감지되었습니다 (빌드 " + info.BuildId + ").\r\n자동으로 검증할 수 없는 파일: " + file + "\r\n한글이 깨진 상태로 실행하지 않았습니다. 최신 한글패치를 확인해주세요. 영문 실행은 계속 사용할 수 있습니다.");
        }
