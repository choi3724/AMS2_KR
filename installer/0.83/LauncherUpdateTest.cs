...
namespace Ams2KoreanBeta
{
    internal static class LauncherUpdateTest
    {
        ...
        private static int Run(string[] args)
        {
            ...
            Require(GithubUpdater.IsNewer("v0.83", "Open Beta 0.81"), "open beta upgrade ordering");
            Require(!GithubUpdater.IsNewer("v0.83", "Open Beta 0.83"), "open beta version equality");
            ...
            File.Copy(args[1], Path.Combine(package, "AMS2 한국어 패치 오픈베타 0.83.exe"));
            Require(File.Exists(GithubUpdater.FindInstaller(package, "v0.83")), "new installer protocol not recognized");
            ...
            {
                Application.EnableVisualStyles(); Application.SetCompatibleTextRenderingDefault(false);
                VerifyStartup(output, "current", Task.FromResult(new GithubReleaseInfo { Tag = "v0.83" }), false, false);
                ...
                VerifyStartup(output, "manual launch", Task.FromResult(new GithubReleaseInfo { Tag = "v0.83" }), false, true);
        ...
        private static void Render(string output, string name, bool vr, GithubReleaseInfo info, float scale)
        {
            using (var form = new LauncherUpdateForm(output, vr))
            {
                ...
                if (info != null) form.OfferUpdate(info, "0.83");
                form.PerformLayout();
                Require(form.Controls.Find("PatchCreator", true)[0].Text == "한글 패치 제작자 : ENGIceBlasT", "creator wording differs");
                Require(form.Controls.Find("PatchVersion", true)[0].Text.Contains("0.83"), "launcher must display 0.83");
