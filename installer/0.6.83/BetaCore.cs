...
namespace Ams2KoreanBeta
{
    ...
    {
        public const string PackageId = "AMS2-KR-BETA-0.6.83-PRETENDARD";
        public const string Version = "Closed Beta 0.6.83";
        ...
        public const long IgphaseHudBytes = 9597262;
        public const string IgphaseHudSourceSha256 = "F967D1A322EB75AAD742CF21888D75DB0CA4CB407ACDEC72F14D32BD5351E7DA";
        public const string IgphaseHudTargetSha256 = "D1618BB1F6E09F53E8BB86F4A163C2934B91814F5F326670381C5496B3D7C398";
        public const long IgphaseHudPatchBytes = 2054;
        public const string IgphaseHudPatchSha256 = "53D939CA71A5FA917B29E8DB615FD443163840FBB9FC943A3B364D4F19F38715";
        ...
        public string IgphaseHudPatchSource { get { return Path.Combine(ReleaseRoot, "payload", "patch", "IGPHASEHUD-entry288.bin"); } }

        public void ValidateAll()
        {
            foreach (DirectFile f in DirectFiles) RequireFile(DirectSource(f), f.Bytes, f.Sha256, "payload");
            RequireFile(IgphaseHudPatchSource, IgphaseHudPatchBytes, IgphaseHudPatchSha256, "IGPHASEHUD minimal patch");
        }
    ...
    internal static class FileOps
    {
        ...
        public static void ApplyIgphaseHudPatch(string patchPath, string target)
        {
            string liveSha = Sha256(target);
            if (liveSha == PackageManifest.IgphaseHudTargetSha256)
            {
                PackageManifest.RequireFile(target, PackageManifest.IgphaseHudBytes, PackageManifest.IgphaseHudTargetSha256, "IGPHASEHUD already patched");
                return;
            }
            PackageManifest.RequireFile(target, PackageManifest.IgphaseHudBytes, PackageManifest.IgphaseHudSourceSha256, "IGPHASEHUD source");
            PackageManifest.RequireFile(patchPath, PackageManifest.IgphaseHudPatchBytes, PackageManifest.IgphaseHudPatchSha256, "IGPHASEHUD minimal patch");
            byte[] patch = File.ReadAllBytes(patchPath);
            string tmp = target + ".krbeta.tmp." + Guid.NewGuid().ToString("N");
            File.Copy(target, tmp, false);
            try
            {
                using (FileStream stream = new FileStream(tmp, FileMode.Open, FileAccess.Write, FileShare.None))
                {
                    stream.Position = 0x3080;
                    stream.WriteByte(patch[0]);
                    stream.Position = 0x3084;
                    stream.WriteByte(patch[1]);
                    stream.Position = 0x3092;
                    stream.Write(patch, 2, 4);
                    stream.Position = 0x8E9800;
                    stream.Write(patch, 6, 2048);
                }
                PackageManifest.RequireFile(tmp, PackageManifest.IgphaseHudBytes, PackageManifest.IgphaseHudTargetSha256, "IGPHASEHUD patched temporary");
                File.Replace(tmp, target, null);
                PackageManifest.RequireFile(target, PackageManifest.IgphaseHudBytes, PackageManifest.IgphaseHudTargetSha256, "IGPHASEHUD patched live");
            }
    ...
    internal sealed class BetaEngine
    {
        ...
        public OperationResult Check(GameInfo game)
        {
            try
            {
                ...
                if (detected == "STOCK_ENGLISH") return Ok(detected, "순정 영문 원본 상태를 확인했습니다.", null);
                if (detected == "RESTORED_EXACT") return Ok(detected, "제거 완료 및 영문 원본 해시 검증 완료", null);
        ...
        public OperationResult Uninstall(GameInfo game)
        {
            try
            {
                ...
                Log("제거 완료: 패치 생성 파일 제거 및 영문 원본 SHA 검증 완료");
                return Ok("RESTORED_EXACT", "영문 원본 복원 완료. 제거 완료와 원복 검증 완료를 모두 확인했습니다.", SaveLog());
            }
        ...
        public OperationResult Diagnose(GameInfo game, string outputZip)
        {
            try
            {
                Prepare(game);
                ValidateGame(game);
                string temp = Path.Combine(Path.GetTempPath(), "AMS2-KR-BETA0683-DIAG-" + Guid.NewGuid().ToString("N"));
                ...
                try
                {
                    ...
                    files.Append(PackageManifest.IgphaseHudRelativePath).Append('\t').Append(File.Exists(bff) ? "1" : "0").Append('\t')
                        ...
                        .Append(File.Exists(bff) ? FileOps.Sha256(bff) : "").Append('\t').Append(PackageManifest.IgphaseHudTargetSha256).AppendLine();
        ...
        private void PreflightFiles()
        {
            ...
            if (!File.Exists(igphaseHud) || new FileInfo(igphaseHud).Length != PackageManifest.IgphaseHudBytes)
                throw new InvalidOperationException("지원되는 IGPHASEHUD.bff가 없습니다.");
            string igphaseHudSha = FileOps.Sha256(igphaseHud);
            if (igphaseHudSha != PackageManifest.IgphaseHudSourceSha256 && igphaseHudSha != PackageManifest.IgphaseHudTargetSha256)
                throw new InvalidOperationException("지원되지 않는 IGPHASEHUD.bff입니다.");
        }
        ...
        private InstallState BuildState(GameInfo game, PreviousInstallInfo previous)
        {
            ...
            DirectState bff = CreateTransactionRecord(PackageManifest.IgphaseHudRelativePath, PackageManifest.IgphaseHudTargetSha256, PackageManifest.IgphaseHudBytes, s.BackupId);
            ResolveCanonicalOriginal(bff, false, new[] { PackageManifest.IgphaseHudSourceSha256 }, history, s.BackupId);
            s.Files.Add(bff);
        ...
        private void ResolveCanonicalOriginal(DirectState target, bool stockIsAbsent, IEnumerable<string> stockHashes, List<PreviousInstallInfo> history, string backupId)
        {
            ...
            foreach (PreviousInstallInfo item in history.OrderBy(x => x.InstalledUtc))
            {
                ...
                if (!stockIsAbsent && old.Action == "modified" && accepted.Contains(old.BeforeSha))
                {
                    ...
                    PackageManifest.RequireFile(source, old.BeforeBytes, old.BeforeSha, "이전 버전 순정 백업");
            ...
            throw new InvalidOperationException("순정 영문 원본을 증명할 수 없습니다: " + target.RelativePath);
        }
        ...
        private void CopyCanonicalBackup(string source, DirectState target, string backupId)
        {
            ...
            if (File.Exists(destination)) PackageManifest.RequireFile(destination, target.BeforeBytes, target.BeforeSha, "순정 원본 백업");
        ...
        private void ApplyFiles(InstallState state)
        {
            ...
            foreach (DirectState d in state.Files)
            {
                ...
                {
                    FileOps.ApplyIgphaseHudPatch(manifest.IgphaseHudPatchSource, PackageManifest.SafeJoin(gameDir, d.RelativePath));
                    continue;
                }
        ...
        private void RestoreOriginal(InstallState state)
        {
            ...
            foreach (DirectState d in state.Files.AsEnumerable().Reverse())
            {
                ...
                if (d.Action == "modified")
                {
                    ...
                    PackageManifest.RequireFile(backup, d.BeforeBytes, d.BeforeSha, "영문 원본 백업");
                    if (File.Exists(live))
                    {
        ...
        private void ValidateBackupContract(InstallState state)
        {
            ...
            foreach (DirectState d in state.Files.Where(x => x.Action == "modified"))
                PackageManifest.RequireFile(PackageManifest.SafeJoin(backupRoot, d.RelativePath), d.BeforeBytes, d.BeforeSha, "영문 원본 백업");
        }
        ...
        {
            bool patchResidue = false;
            foreach (DirectFile f in manifest.DirectFiles)
            {
                string live = PackageManifest.SafeJoin(gameDir, f.RelativePath);
                if (!File.Exists(live)) { if (f.Role == "modified") patchResidue = true; continue; }
                string sha = FileOps.Sha256(live);
                if (f.Role == "modified")
                {
                    if (sha == f.Sha256) patchResidue = true;
                    else if (!f.AllowedBefore.Contains(sha, StringComparer.OrdinalIgnoreCase)) return "MANUAL_CHANGES_DETECTED";
                }
                else
                {
                    if (sha == f.Sha256 || f.AllowedBefore.Contains(sha, StringComparer.OrdinalIgnoreCase)) patchResidue = true;
                    else return "MANUAL_CHANGES_DETECTED";
                }
            }
            string bff = PackageManifest.SafeJoin(gameDir, PackageManifest.IgphaseHudRelativePath);
            if (!File.Exists(bff)) return "MIXED_OR_INCOMPLETE";
            string bffSha = FileOps.Sha256(bff);
            if (bffSha == PackageManifest.IgphaseHudTargetSha256) patchResidue = true;
            else if (bffSha != PackageManifest.IgphaseHudSourceSha256) return "MANUAL_CHANGES_DETECTED";
            return patchResidue ? "MIXED_OR_INCOMPLETE" : "STOCK_ENGLISH";
        }
