...
namespace Ams2KoreanBeta
{
    ...
    {
        public const string PackageId = "AMS2-KR-BETA-0.6.86-PRETENDARD";
        public const string Version = "Closed Beta 0.6.86";
    ...
    internal sealed class BetaEngine
    {
        ...
        public OperationResult Diagnose(GameInfo game, string outputZip)
        {
            try
            {
                Prepare(game);
                ValidateGame(game);
                string temp = Path.Combine(Path.GetTempPath(), "AMS2-KR-BETA0686-DIAG-" + Guid.NewGuid().ToString("N"));
        ...
        private void PreflightFiles()
        {
            ...
            string tempRoot = Path.Combine(Path.GetTempPath(), "AMS2-KR-BETA0686-BFF-" + Guid.NewGuid().ToString("N"));
        ...
        private string InspectUnmanagedState()
        {
            ...
            bool adoptableCreatedResidue = false;
            foreach (DirectFile f in manifest.DirectFiles)
            {
                ...
                if (f.Role == "modified")
                {
                    if (sha == f.Sha256) unsafeResidue = true;
                    else if (!f.AllowedBefore.Contains(sha, StringComparer.OrdinalIgnoreCase)) return "MANUAL_CHANGES_DETECTED";
                }
                else
                {
                    if (sha == f.Sha256 || f.AllowedBefore.Contains(sha, StringComparer.OrdinalIgnoreCase)) adoptableCreatedResidue = true;
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (route.Equals("GUI\\kr13_driver_name_semibold.bfont", StringComparison.OrdinalIgnoreCase) ||
                        route.Equals("GUI\\kr13_font_hud_main.bfont", StringComparison.OrdinalIgnoreCase)) unsafeResidue = true;
            ...
            return adoptableCreatedResidue ? "ADOPTABLE_CREATED_RESIDUE" : "STOCK_ENGLISH";
        }
        ...
        private void CleanupIgphaseHudCandidate()
        {
            ...
            try
            {
                ...
                if (directory.StartsWith(temp, StringComparison.OrdinalIgnoreCase) && Path.GetFileName(directory).StartsWith("AMS2-KR-BETA0686-BFF-", StringComparison.Ordinal))
