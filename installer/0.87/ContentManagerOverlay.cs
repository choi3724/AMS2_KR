...
namespace Ams2KoreanBeta
{
    ...
    internal static class ContentManagerOverlay
    {
        ...
        internal static byte[] Patch(byte[] source, Edit edit, bool reverse)
        {
            ...
            int offset = -1;
            if (edit.offset >= 0 && edit.offset <= source.Length - from.Length && source.Skip(edit.offset).Take(from.Length).SequenceEqual(from)) offset = edit.offset;
            else
            {
                for (int at = 0; at <= source.Length - from.Length; at++)
                {
                    if (!source.Skip(at).Take(from.Length).SequenceEqual(from)) continue;
                    if (offset >= 0) throw new InvalidDataException("CM 한글 변경 영역이 여러 곳이라 안전하게 적용할 수 없습니다.");
                    offset = at;
                }
            }
            if (offset < 0)
            {
                int applied = 0;
                for (int at = 0; at <= source.Length - to.Length; at++) if (source.Skip(at).Take(to.Length).SequenceEqual(to)) applied++;
                if (applied == 1) return (byte[])source.Clone();
