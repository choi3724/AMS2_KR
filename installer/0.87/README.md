...
0.87은 구버전 설치 기록과 정식 배포 파일을 확인해 중간 버전 없이 최신 패치로 업데이트한다. 게임 빌드 번호를 참고 정보로만 사용하는 0.86 정책도 유지한다.
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File tools/repository/Build-InstallerOutsideRepo.ps1 -Version 0.87 -WorkRoot <작업 경로> -CompatibilityDataRoot <검증된 호환성 데이터> -VerifiedBffPatcher <정식 0.86의 변경 없는 BFF 적용기>
```

정식 배포 식별 목록 생성과 검증 범위는 `docs/COMMON_UPGRADE_0.87.md`에 기록한다.
