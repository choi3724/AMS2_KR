...
namespace Ams2KoreanBeta
{
    internal static class LauncherUpdateTest
    {
        ...
        private static int Run(string[] args)
        {
            ...
            Refuses(() => GithubUpdater.ParseVersion("v0.9-test1"), "prerelease accepted");
            string json = "{\"tag_name\":\"v0.9\",\"html_url\":\"https://github.com/choi3724/AMS2_KR/releases/tag/v0.9\",\"draft\":false,\"prerelease\":false,\"author\":{\"html_url\":\"https://example.com\"},\"assets\":[{\"name\":\"AMS2.CB.0.9.zip\",\"size\":100,\"digest\":\"sha256:" + new string('a', 64) + "\",\"browser_download_url\":\"https://github.com/choi3724/AMS2_KR/releases/download/v0.9/AMS2.CB.0.9.zip\"}]}";
            ...
            File.Copy(args[1], Path.Combine(package, "AMS2-Korean-Patch-CB-0.8.exe"));
            Require(File.Exists(GithubUpdater.FindInstaller(package, "v0.8")), "new installer protocol not recognized");
            Refuses(() => GithubUpdater.FindInstaller(package, "v0.9"), "installer version mismatch accepted");
            ...
            {
                Application.EnableVisualStyles(); Application.SetCompatibleTextRenderingDefault(false);
                VerifyStartup(output, "current", Task.FromResult(new GithubReleaseInfo { Tag = "v0.8" }), false, false);
                ...
                VerifyStartup(output, "manual launch", Task.FromResult(new GithubReleaseInfo { Tag = "v0.8" }), false, true);
        ...
        private static void Render(string output, string name, bool vr, GithubReleaseInfo info, float scale)
        {
            using (var form = new LauncherUpdateForm(output, vr))
            {
                ...
                if (info != null) form.OfferUpdate(info, "0.8");
                form.PerformLayout();
