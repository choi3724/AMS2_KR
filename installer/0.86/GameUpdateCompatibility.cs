    ...
    internal static class GameUpdateCompatibility
    {
        ...
        internal static CompatibilityOutcome Ensure(string game, bool requireInstalled, Action<string> log = null)
        {
            ...
            using (var mutex = new Mutex(true, @"Local\AMS2-Korean-GameUpdate-" + FileOps.Sha256Text(Path.GetFullPath(game).ToUpperInvariant()).Substring(0, 24), out owner))
            {
                ...
                int optionalCount = rules.policy == null || rules.policy.optionalArchives == null ? 0 : rules.policy.optionalArchives.Length;
                bool currentContract = rows.Count >= rules.files.Count - optionalCount && rows.Count <= rules.files.Count;
                bool legacyContract = rows.Count == 450;
                if ((!currentContract && !legacyContract) || rows.Any(x => x.Length != 9) || rows.Select(x => x[0].Replace('\\', '/').ToLowerInvariant()).Distinct().Count() != rows.Count)
                ...
                foreach (string[] row in rows)
                {
                    ...
                    if (!rules.files.ContainsKey(relative)) { failures.Add(relative + " | 관리 범위 밖 설치 기록"); continue; }
