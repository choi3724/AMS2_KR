...
namespace Ams2KoreanBeta
{
    internal static class LauncherUpdateTest
    {
        ...
        private static int Run(string[] args)
        {
            ...
            Require(GithubUpdater.IsNewer("v0.86", "Open Beta 0.81"), "open beta upgrade ordering");
            Require(!GithubUpdater.IsNewer("v0.86", "Open Beta 0.86"), "open beta version equality");
            Refuses(() => GithubUpdater.ParseVersion("not-a-version"), "malformed version accepted");
            Refuses(() => GithubUpdater.ParseVersion("v0.86-test1"), "prerelease accepted");
            string json = "{\"tag_name\":\"v0.86\",\"html_url\":\"https://github.com/choi3724/AMS2_KR/releases/tag/v0.86\",\"draft\":false,\"prerelease\":false,\"author\":{\"html_url\":\"https://example.com\"},\"assets\":[{\"name\":\"AMS2.0.86.zip\",\"size\":100,\"digest\":\"sha256:" + new string('a', 64) + "\",\"browser_download_url\":\"https://github.com/choi3724/AMS2_KR/releases/download/v0.86/AMS2.0.86.zip\"}]}";
            GithubReleaseInfo release = GithubUpdater.ParseRelease(json);
            Require(GithubUpdater.ParseRelease(json.Replace("AMS2.0.86.zip", "AMS2.CB.0.86.zip")).AssetUrl != null, "legacy ZIP naming rejected");
            ...
            File.Copy(args[1], Path.Combine(package, "AMS2 한국어 패치 오픈베타 0.86.exe"));
            Require(File.Exists(GithubUpdater.FindInstaller(package, "v0.86")), "new installer protocol not recognized");
            ...
            {
                Application.EnableVisualStyles(); Application.SetCompatibleTextRenderingDefault(false);
                VerifyStartup(output, "current", Task.FromResult(new GithubReleaseInfo { Tag = "v0.86" }), false, false);
                ...
                VerifyStartup(output, "manual launch", Task.FromResult(new GithubReleaseInfo { Tag = "v0.86" }), false, true);
        ...
        private static void Render(string output, string name, bool vr, GithubReleaseInfo info, float scale)
        {
            using (var form = new LauncherUpdateForm(output, vr))
            {
                ...
                if (info != null) form.OfferUpdate(info, "0.86");
                form.PerformLayout();
                Require(form.Controls.Find("PatchCreator", true)[0].Text == "한글 패치 제작자 : ENGIceBlasT", "creator wording differs");
                Require(form.Controls.Find("PatchVersion", true)[0].Text.Contains("0.86"), "launcher must display 0.86");
