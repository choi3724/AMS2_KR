...
namespace Ams2KoreanBeta
{
    ...
    {
        public const string PackageId = "AMS2-KR-BETA-0.86-PRETENDARD";
        public const string Version = "Open Beta 0.86";
    ...
    internal sealed class BetaEngine
    {
        ...
        public OperationResult Install(GameInfo game, ShortcutOptions shortcuts)
        {
            ...
            try
            {
                ...
                manifest.ValidateAll();
                GuardProcesses();
                GameUpdateCompatibility.CompatibilityOutcome compatibility = GameUpdateCompatibility.Ensure(gameDir, false, Log);
                string detected = DetectState();
                ...
                string path = SaveLog();
                bool warning = preflightWarnings.Count > 0 || compatibility.SkippedOptional > 0 || compatibility.Warnings.Count > 0;
                string warningText = warning ? " 일부 부가 표시 항목은 안전을 위해 생략했습니다. 핵심 한글 자산은 적용되었으며 한글 모드 실행이 가능합니다." : "";
                if (previous != null)
            ...
            catch (Exception e)
            {
                ...
                return Fail("INSTALL_FAILED", e.Message + (rollbackAttempted ? " (자동 롤백 완료)" : ""));
            }
        ...
        private void PreflightFiles()
        {
            ...
            candidateRoot = Path.Combine(Path.GetTempPath(), "AMS2-KR-BETA086-BFF-" + Guid.NewGuid().ToString("N"));
            ...
            foreach (DirectFile f in manifest.DirectFiles.Where(x => x.Role == "modified" && x.RelativePath.EndsWith(".bgui", StringComparison.OrdinalIgnoreCase)))
            {
                ...
                try
                {
                    ...
                    byte[] candidate = GameUpdateCompatibility.PatchMenu(File.ReadAllBytes(PackageManifest.SafeJoin(gameDir, f.RelativePath)), rule);
            ...
            foreach (DirectFile f in manifest.DirectFiles.Where(x => x.Role == "created" && x.RelativePath.EndsWith(".bgui", StringComparison.OrdinalIgnoreCase)))
            {
                string target = PackageManifest.SafeJoin(gameDir, f.RelativePath);
                if (!File.Exists(target) || FileOps.Sha256(target) == f.Sha256) continue;
                string key = f.RelativePath.Replace('\\', '/').ToLowerInvariant();
                try
                {
                    byte[] candidate;
                    if (!cmOwned.Contains(key) || !ContentManagerOverlay.TryPatchCurrent(key, File.ReadAllBytes(target), out candidate))
            ...
            foreach (DirectFile f in manifest.DirectFiles.Where(x => x.Role == "created" && !x.RelativePath.EndsWith(".bgui", StringComparison.OrdinalIgnoreCase)))
            {
                ...
                string current = FileOps.Sha256(target);
                if (current == f.Sha256 || f.AllowedBefore.Contains(current)) continue;
                failures.Add(f.RelativePath + " | 기존 생성 파일이 한글패치 소유 파일과 충돌합니다.");
            }
            if (failures.Count > 0) throw new InvalidOperationException("필수 한글 파일 후보 생성에 실패했습니다. 게임 파일은 변경하지 않았습니다.\r\n" + String.Join("\r\n", failures.Take(40)));
            try
            {
        ...
        private void CleanupIgphaseHudCandidate()
        {
            ...
            try
            {
                ...
                if (directory.StartsWith(temp, StringComparison.OrdinalIgnoreCase) && Path.GetFileName(directory).StartsWith("AMS2-KR-BETA086-BFF-", StringComparison.Ordinal))
