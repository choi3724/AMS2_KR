...
namespace Ams2KoreanBeta
{
    ...
    {
        public const string PackageId = "AMS2-KR-BETA-0.6.87-PRETENDARD";
        public const string Version = "Closed Beta 0.6.87";
    ...
    internal sealed class BetaEngine
    {
        ...
        public OperationResult Diagnose(GameInfo game, string outputZip)
        {
            try
            {
                Prepare(game);
                ValidateGame(game);
                string temp = Path.Combine(Path.GetTempPath(), "AMS2-KR-BETA0687-DIAG-" + Guid.NewGuid().ToString("N"));
        ...
        private void PreflightFiles()
        {
            ...
            string tempRoot = Path.Combine(Path.GetTempPath(), "AMS2-KR-BETA0687-BFF-" + Guid.NewGuid().ToString("N"));
        ...
        private void CleanupIgphaseHudCandidate()
        {
            ...
            try
            {
                ...
                if (directory.StartsWith(temp, StringComparison.OrdinalIgnoreCase) && Path.GetFileName(directory).StartsWith("AMS2-KR-BETA0687-BFF-", StringComparison.Ordinal))
