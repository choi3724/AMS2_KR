...
업데이트 설치와 설치 후 파일 검증을 통과하면 기존 업데이트 창에 `업데이트가 완료되었습니다. 게임을 실행합니다.`를 약 2초간 표시한 뒤 게임을 실행한다. 일반·VR 업데이트에 공통 적용되며, 추가 확인 버튼은 필요 없다. 설치나 검증이 실패하면 완료 안내와 게임 실행으로 넘어가지 않는다.
